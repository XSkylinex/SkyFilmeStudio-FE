...
| 07 | [Asset ingestion & subject review](07-assets-and-subjects.md) | 06 | BE-11, BE-12 | **partly done 2026-08-21, detail view and subject review 2026-08-22** · the asset library, capture guide, **asset detail** and now the **subject list and Subject Review** are real. BE-12 published `subjects` and `canonical-sets`, so a subject's identity, its approved set's lineage — version, frozen descriptor and its SHA-256 — and its references with anchor eligibility all render. **Registration and both import transports have endpoints whose request shapes are not published through `./contracts`**, so each says so on screen rather than offering a control that does nothing — every request and query schema lives in a `*/dto/*.schema.ts` under a feature module and the contracts barrel re-exports none of them. **Approval was never one of them**: `POST :setId/approve` takes no body at all, and what blocked it was a missing `GET` on the collection, so no draft could be rediscovered after a reload. `edb38a3` added that route on 2026-08-22, drafts included, which unblocks the approval half and the draft-versus-approved comparison with it. That is the next buildable work in this repo |
| 08 | [Style, voice, location & prop studio](08-style-studio.md) | 07 | BE-13 | **blocked: BE-13 in progress, no route yet** — corrected 2026-08-22; the previous entry read `not started`, which the backend's plan table still says and its branch contradicts. `be-13-styles-voices-locations-props` now carries `styleModeSchema`, the lineage and versioning migrations, an immutability trigger, `StyleProfilesRepository` and two new error codes. `StyleProfilesRepository` is now a registered provider, but there is still **no controller and no route** for style profiles, voices, locations or props. The backend session's own order is style profiles → voices → pronunciations → locations → props, and it expects only style profiles callable this session — one screen out of seven is not a phase. What this repo did build is the ungated half: the vocabulary guard and the two error codes |
