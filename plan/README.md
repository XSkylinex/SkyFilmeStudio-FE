...
| 10 | [Storyboard review](10-storyboard.md) | 09 | BE-18 | **blocked: nothing bootstraps a `SceneId`** — BE-18 is no longer a blocker and the phase has not moved. Its status changed twice inside one session: `not started` (2026-08-31, from the backend's own table), then `in progress` when the backend was found on `be-18-storyboard-and-keyframes` with four refusals declared and unmerged, then **merged as `f14098e` on 2026-09-01**, publishing the whole storyboard surface — frames, regeneration, framing and expression edits, keyframe waiver, requirement and status, plus frame comparison and approval — with all five of its refusals wired to real throw sites. Every one of those routes is keyed on a `shotId`. **Track the missing route, not the phase number.** — measured 2026-08-31, re-measured 2026-09-01. BE-16 merged as `d658f69` and published a real `shots.controller.ts`, but every one of its routes is keyed on a scene or shot id this repo cannot obtain: the contracts are complete — `sceneSchema` carries `id: sceneIdSchema` and `shotSchema` carries `sceneId` — but **nothing bootstraps the first id**. Every route returning a `Shot`, `Scene` or `DialogueLineRecord` sits under `scenes/:sceneId/…`, `shots/:id` or `dialogue-lines/:id`; `PUT /planning/scenes` writes the outline and returns `readonly unknown[]`, discarding the ids, and there is no `GET`. The chain is circular, so **the missing piece is one `GET` that returns a production's scenes** |
