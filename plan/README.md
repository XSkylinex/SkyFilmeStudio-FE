...
| 12 | [Shot review](12-shot-review.md) | 11 | BE-19, BE-20 | **blocked: three separate reasons, measured 2026-09-01** — and "not started" was the wrong word for all of them. The routes landed: `shots/:shotId/qc` publishes `GET runs`, `POST request-review` and `POST review`, and `shots/:shotId/video` publishes all five §44 regeneration modes. What stops the screen is, first, that **no route serves an artifact's bytes** — `src/artifacts/` has a module and a repository and no controller, and the only `StreamableFile` routes in the orchestrator are the source asset thumbnail and proxy — so the `canonical | first | middle | last` comparison the phase exists for cannot be rendered at all; second, that **`reviewShotRequestSchema` and `submitVideoRenderRequestSchema` are not re-exported by `src/contracts/index.ts`**, so the human decision cannot be recorded and no regeneration can be submitted; and third, that a hero shot cannot be identified: `HERO` is a published `qualityIntent` on a `RenderProfile`, and `Shot` carries a `renderProfileId`, but `src/render-profiles/` has a module, an entity and a repository and **no controller**, so nothing resolves that id. The concept exists and is simply unreachable — which is a missing route, not a missing shape, and would move the day one `GET` is added. `qcRunSchema`, `shotReviewSchema`, the sixteen `QcCheckId` technical checks and the eight `subjectQcRule` verdicts *are* published, so the advisory half becomes buildable the day any one of the three is fixed |
| 13 | [Audio & music](13-audio.md) | 09 | BE-21 | **partly done 2026-09-01** — the dialogue half is real and BE-21 gated none of it. `src/contracts/index.ts` carries an explicit block publishing the three dialogue-line DTOs, `synthesiseSpeechRequestSchema`, `chooseDialogueTierRequestSchema` and the dialogue-timing report, so `/productions/:id/audio` now reads a production's scenes, the lines each carries, and every take a line has produced with the provenance §18.2 records. **It writes**: a line is synthesised in either pass, its audio is approved and un-approved, and its animation tier is chosen. What had made all of it unreachable was the same single route that blocked FE-10 — nothing produced a `sceneId` until `dcf6d49`. **Track the missing route, not the phase number**, for the second phase running. The largest thing it landed is not in the phase's own box list: `POST /productions/:id/dialogue-timing` replaces FE-09's typed-in durations with measured ones and says which scenes are still estimates rather than averaging the two. Unbuilt for three distinct reasons — the OST and cue work is on an unmerged `be-21-audio` branch; the SFX library, stems, mix and loudness have no controllers at all; and **nothing can be played**, because no route in the orchestrator serves an artifact's bytes |
