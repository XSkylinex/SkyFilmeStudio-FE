...
| 07 | [Asset ingestion & subject review](07-assets-and-subjects.md) | 06 | BE-11, BE-12 | **partly done 2026-08-21, detail view 2026-08-22** · the asset library, the capture guide and now the **asset detail view** are real, against `/projects/:projectId/assets`, `/capture-guide` and `/projects/:projectId/assets/:assetId`. The detail view carries the checksum, the probe metadata as the orchestrator wrote it, and the **scrub proxy — the first `<video>` in this app**, on a `lazy` route so it builds as its own chunk. Derived assets and referencing subjects have no endpoint. Both import transports exist but **neither request shape is published through `./contracts`**, so importing stays unwired rather than hand-typed. Subjects and canonical sets wait on **BE-12**, which has entities but no controller |
| 08 | [Style, voice, location & prop studio](08-style-studio.md) | 07 | BE-13 | **blocked: BE-13 not started** — re-checked 2026-08-22 in the backend's own plan table, which reads `not started` for BE-13 and for every phase after it. `StyleProfile`, `VoiceProfile`, `Location` and `Prop` are all published through `./contracts`, so the shapes are real — but only their **entities** exist in the backend. No service, no controller, no module in `app.module.ts`, and no route serves any of the four. Shapes with no server |
