...
| 07 | [Asset ingestion & subject review](07-assets-and-subjects.md) | 06 | BE-11, BE-12 | **partly done 2026-08-21** · the asset library and the capture guide are real, against `/projects/:projectId/assets` and `/capture-guide`. Both import transports exist as endpoints but **neither request shape is published through `./contracts`**, so importing stays unwired rather than hand-typed. Subjects, canonical sets and Subject Review wait on **BE-12**, which has entities but no controller |
